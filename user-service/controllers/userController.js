...
exports.getUserCourses = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: `Error fetching user courses: ${err.message}` });
  }
};
...
exports.getUserCompletedCourses = async (req, res) => {
  try {
    ...
    const coursePromises = courseIds.map((courseId) =>
      axios.get(`http://course-service:5002/api/v1/courses/${courseId}`, {
        headers: {
    ...
    res
      .status(500)
      .json({ message: `Error fetching completed courses: ${error.message}` });
  }
};
