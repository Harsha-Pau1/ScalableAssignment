...
exports.getCourseById = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: `Error fetching course: ${error.message}` });
  }
};
...
exports.getUsersEnrolledInCourse = async (req, res) => {
  try {
    ...
    const userPromises = userIds.map((userId) =>
      axios.get(`http://user-service:5001/api/v1/users/${userId}`, {
        headers: {
    ...
    res
      .status(500)
      .json({ message: `Error fetching enrolled users: ${error.message}` });
  }
};
...
exports.enrollStudentInCourse = async (req, res) => {
  ...
  try {
    ...
    const userResponse = await axios.get(
      `http://user-service:5001/api/v1/users/${userId}`,
      {
        headers: {
    ...
    if (user.role !== "Student") {
      return res
        .status(403)
        .json({ message: "Only students can be enrolled in courses" });
    }
    ...
    if (
      ...
      user.coursesEnrolled.includes(courseId)
    ) {
      return res
        .status(409)
    ...
    res
      .status(500)
      .json({ message: `Error enrolling user in course: ${error.message}` });
  }
};
...
exports.getCoursesByLevel = async (req, res) => {
  try {
    ...
    if (!allowedLevels.includes(level)) {
      return res.status(400).json({
        message:
          "Invalid level parameter. Allowed values are: Beginner, Intermediate, Advanced.",
      });
    }
    ...
    res
      .status(500)
      .json({ message: `Error fetching courses by level: ${error.message}` });
  }
};
...
exports.deenrollStudentFromCourse = async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ message: "User de-enrolled from course successfully" });
  } catch (error) {
    if (error.response && error.response.status === 404) {
      return res.status(404).json({ message: "User not found" });
    }
    res.status(500).json({
      message: `Error de-enrolling user from course: ${error.message}`,
    });
  }
};
